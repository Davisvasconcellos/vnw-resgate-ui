...
export default function BottomNavBoat() {
  const pathname = usePathname()
  const { t } = useI18n()

  const isNearby = pathname === '/nearby'
  const isRoutes = pathname === '/routes-boats' || pathname === '/routes'
    ...
    <nav className="fixed bottom-0 left-0 w-full z-50 bg-white/95 dark:bg-[#070d17]/95 backdrop-blur-xl border-t border-slate-100 dark:border-white/10 shadow-[0_-8px_32px_rgba(0,0,0,0.06)] dark:shadow-none pb-safe">
      <div className="flex items-center justify-around px-4 py-3 max-w-lg mx-auto relative">
        ...
        <div className="relative -top-10">
          <Link
            href="/routes-boats"
            className="w-16 h-16 rounded-full flex items-center justify-center active:scale-90 transition-all border-4 border-white dark:border-[#0a1628] shadow-xl bg-[#0277BD] text-white shadow-[#0277BD]/40"
          >
            <span className="material-symbols-outlined text-[32px] font-bold">directions_boat</span>
          </Link>
          <div className="absolute top-0 right-0 w-5 h-5 bg-error text-white text-[10px] font-black rounded-full border-2 border-white dark:border-[#0a1628] flex items-center justify-center shadow-sm">1</div>
        </div>

        {/* Maps/Nearby Button */}
        <Link
          href="/nearby?module=boat"
          className={`flex flex-col items-center gap-1.5 p-2 min-w-[70px] transition-all active:scale-90 group ${
            isNearby ? 'text-[#0277BD] dark:text-blue-400' : 'text-on-surface-variant dark:text-slate-400'
          }`}
        >
          <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-colors ${
            isNearby ? 'bg-blue-50 dark:bg-blue-900/20 text-[#0277BD] dark:text-blue-400' : 'group-hover:bg-blue-50/50 dark:group-hover:bg-white/5'
          }`}>
            <span 
